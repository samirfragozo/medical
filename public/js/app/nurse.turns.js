...
function getStatus(column, value) {
    if (column === 3) {
        ...
        if (value['state'] === 'ATENDIDO') return dropdown;
        if (value['state'] === 'EN PROGRESO') return (
            dropdown +
            '<a onclick="state(' + value['id'] + ',\'' + value['next'] + '\')" class="m-portlet__nav-link btn m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--hover-success" title="Terminar Turno">' +
